...
//move the eyes randomly every 3 seconds
...
const moveEye = function(tag, mouseX, mouseY){
  ...
  const difX = mouseX - eyeMidX
  const difY = mouseY - eyeMidY - window.pageYOffset

  const dif = Math.sqrt(difX * difX + difY * difY)

  //get capped radius

  const radius = Math.min(3, dif)

  const angle = Math.atan2(difY, difX)

  //get capped version of X based off angle
  cappedX = radius * Math.cos(angle)
  cappedY = radius * Math.sin(angle)
